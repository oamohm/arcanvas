...
function Sparkline({ data = [], color = '#00a5ff', w = 100, h = 32 }) {
  ...
  const gradId = `sgl${color.replace('#', '')}`;
  return (
...
function StatCard({ label, value, sub, delta, chart, color = '#00a5ff' }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-4 flex flex-col gap-3">
      <div className="text-zinc-600 uppercase tracking-widest font-mono text-sm">{label}</div>
      <div className="flex items-end justify-between gap-2">
        ...
        {delta && (
          <div className={`text-xs font-mono mt-1 ${up ? 'text-green-500' : 'text-red-500'}`}>
            {up ? '▲' : '▼'} {delta}
          </div>
        )}
      </div>
...
export default function AssetOverview({ balances = {}, address, tick }) {
  ...
  const usdcChart = Array.from({ length: 16 }, (_, i) => 4.5 + Math.random() * 0.8);
